...
class AgentCard extends Model
{
	    ...
	    public function agentchile($data)
    {
       ...
       if(array_key_exists('start_time', $data) && !array_key_exists('end_time', $data) && $data['start_time'] !='' && $data['end_time'] !='')
        ...
        $sql =  "select * from (select id,card_num,wx_name,all_fee as all_card,status,created_at,all_return_fee as return_fee from hand_agent ".$where."  order by created_at desc )   agentinfo limit ".$start.",".$limit;
...
/*            $monthlog = db('month_log')->where(['agent_id' => $data['id']])->find();
            ...
            if (!$result) {
                return return_json(2, '下单失败');
            }
            $return['fee_num'] = $update['fee_num'];
            $return['order_code'] = $update['order_code'];
            //执行给用户加房卡（暂时不用）
            // 提交事务
            Db::commit();
            return return_json(1, '下单成功',$return);
        } catch (\Exception $e) {
                // 回滚事务
                Db::rollback();
                return return_json(2,'下单失败');
